    ...
    try {
       ...
       else // shouf yakma deja kayn f tournoi
       {
        ...
        else if (Guest3 == -1) // bash kanssifto notification dyal semi final 1
        {
            ...
            sendMessage(Hostsocket , HostConvId  , `You are successfully registered in the tournament: ${tournament.name}, your match will start soon!` );
            sendMessage(Guestsocket , Guest1ConvId  , `You are successfully registered in the tournament: ${tournament.name}, your match will start soon!`);
            ...
            firstMsg.run( HostConvId,`You are successfully registered in the tournament: ${tournament.name}, your match will start soon!`);

            const secondMsg = request.server.db.prepare("INSERT INTO bot_room (conversation_id ,message  ) VALUES (? , ?)");
            secondMsg.run( Guest1ConvId,`You are successfully registered in the tournament: ${tournament.name}, your match will start soon!`);
        }
        else {
            return reply.code(400).send({
                success: false,
...
export async function saveTournamentMatch(request, reply) {
    ...
    try {
        ...
        else if (existingMatchesCount == 0) // bash nssifto message dyal semi final 2
        {
            ...
            sendMessage(Guest2socket , HostConvId  , `You are successfully registered in the tournament: ${tournament.name}, your match will start soon!` );
            sendMessage(Guest3socket , Guest1ConvId  , `You are successfully registered in the tournament: ${tournament.name}, your match will start soon!`);
  ...
  const mailOptions = {
    ...
    html: `   <!DOCTYPE html>
                ...
                <div class="container">
                    ...
                    <p>Enter the following verification code to proceed. This code is valid for <strong>5 minutes</strong>:</p>
