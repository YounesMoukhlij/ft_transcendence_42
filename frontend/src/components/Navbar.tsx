...
function SearchBar() {
  ...
  const { friends, sentRequests, pendingRequests, addSentRequests } = useUserStore();
...
function NotificationsIcon() {
  ...
  function showNotification() {
    ...
    try {
      axios.post(
        `https://${process.env.NEXT_PUBLIC_BACKENDIP}:${process.env.NEXT_PUBLIC_BACKENDPORT}/api/NotificationSeen`,
        {},
        { headers: { Authorization: `Bearer ${user.access_token}` } }
      );
    } catch (err) {
  ...
  async function DelteFriendRequest(notify_id) {
    ...
    const res = await axios.delete(
      `https://${process.env.NEXT_PUBLIC_BACKENDIP}:${process.env.NEXT_PUBLIC_BACKENDPORT}/api/DeleteFriendRequest`,
      {
        params: { id: notify_id },
        headers: { Authorization: `Bearer ${user.access_token}` }
      }
  ...
  async function AcceptFriendRequest(item: friendRequestType) {
    const res = await axios.post(
      `https://${process.env.NEXT_PUBLIC_BACKENDIP}:${process.env.NEXT_PUBLIC_BACKENDPORT}/api/AddFriend`,
      {
        id: item.sender_user,
      },
      {
...
export default function Navbar() {
  ...
  const { addPendingRequestsArray, setnotifications } = useUserStore();
  const user = useUserStore((state) => state.user);
