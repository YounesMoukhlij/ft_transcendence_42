...
export default function Player2Setup() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setPlayers([
      gameState.players[0] || { name: 'Player 1', avatar: '' },
      { name: name || 'Player 2', avatar: avatar || '' },
    ]);
          ...
          <div className="flex gap-4 mb-2">
            {predefinedAvatars.map(src => (
              <img
                ...
                src={src}
                alt="avatar"
                className={`w-16 h-16 rounded-full border-2 cursor-pointer ${avatar === src ? 'border-blue-400' : 'border-gray-500'}`}
