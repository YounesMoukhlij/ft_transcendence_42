...
const TeamBox = ({isFinals = false,  isWinner, score, name, id }: {isFinals?: boolean,  isWinner : boolean, score: number, name: string, id: number }) => {
  ...
  return (
    ...
    <div className={`relative group z-10 transition-transform duration-300 hover:scale-105 ${isFinals ? 'w-64' : 'w-56'}`}>
      ...
      `}>
        <span onClick={() => {router.push(`/profile/${id}`)}} className="text-sm tracking-wide uppercase font-black truncate hover:text-primary cursor-pointer">{name}</span>
  ...
  return (
    <div className="flex flex-col justify-center items-center mx-4 h-full relative ">
      ...
      <div className="z-20 text-center space-y-0.5 ml-10 bg-slate-900/90 p-2 rounded border border-primary backdrop-blur-sm shadow-xl">
        <div className="text-[10px] font-bold text-primary tracking-wider uppercase">{gameLabel === "Semifinal" ? t('game.semiFinal') : t('game.final')}</div>
...
export default function TournamentBracket() {
  ...
  return (
    <div className="h-[84vh] mt-4 flex rounded-lg  bg-[#1a1f2e]/40 text-white font-sans  flex-col md:flex-row lg:flex-row ">
      
      <div className="relative md:min-w-[27%] lg:min-w-[27%] flex flex-col items-center justify-center p-8 border-r border-white/5 overflow-hidden">
     ...
     <div className="space-y-1">
            <h1 className="text-5xl md:text-3xl lg:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white to-slate-400 uppercase tracking-tighter transition-all ease-in-out duration-300 ">
              {tournamentData[0].tournamentName.length > 8 ? tournamentData[0].tournamentName.substring(0, 8)+ " ..." : tournamentData[0].tournamentName}<br/>{t('game.tournament')}
            </h1>
          </div>
        </div>
      </div> 
      <div className="relative flex items-center  p-4 md:p-12 bracket ">
               ...
               <div key={index} className="relative flex items-center">
                <div className="flex flex-col gap-12">
                   <div className="relative">
                      <TeamBox name={match.host} score={match.host_score} isWinner={match.winner == match.host_id} id={match.host_id} />
                   </div>

                   <div className="relative">
                      <TeamBox  name={match.guest} score={match.guest_score} isWinner={match.winner == match.guest_id} id={match.guest_id} />
                   </div>
                </div>
              ...
              <div className="bg-sky-100 p-1 mt-[17%] rounded-sm shadow-[0_0_25px_#3b82f6] transition-transform duration-300 hover:scale-102 ">
                 <div className="bg-slate-900 border border-primary p-4 w-64 text-center relative overflow-hidden">
                    ...
                    
                   {
                     ...
                     <>
                     <span  onClick={() => {router.push(`/profile/${tournamentData[2].winner}`)}} className="block text-xl font-black text-white uppercase hover:text-primary cursor-pointer">{tournamentData[2].winner_name || "TBD"}</span>
                     </>
                     : 
