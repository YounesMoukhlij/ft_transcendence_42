...
export default function RemoteGamePage() {
  ...
  // Fetch only friends from API (users with friendship relationship)
  const fetchFriends = async () => {
    const username = (globalStore.getState() as GlobalStoreType).username;
    if (!username) {
      console.warn('No username available for fetching friends');
      return;
    }

    setLoadingFriends(true);
    try {
      const response = await fetch(`http://localhost:4444/GetFriends?username=${encodeURIComponent(username)}`);
      if (response.ok) {
        const friends = await response.json();
        console.log(`Fetched ${friends.length} friends for user ${username}`);

        // Transform the data to match the expected format
        // Only includes users who have established friendship in the friends table
        const transformedFriends = friends.map((friend: any) => ({
          id: friend.id_user.toString(),
          name: friend.fullname || friend.username,
          avatar: friend.profile_img || '/api/placeholder/32/32',
          isOnline: Boolean(friend.status)
        }));
        setFriendsList(transformedFriends);
      } else {
  ...
  const sendInvitationToFriend = (friend: any) => {
    ...
    alert(`Game invitation sent to ${friend.name}!`);
  ...
  useEffect(() => {
    ...
    fetchFriends(); // Fetch friends when component mounts
          ...
          <div className="space-y-4">
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                onClick={() => {
                  if (!showFriendsDropdown) {
                    fetchFriends(); // Refresh friends when opening dropdown
                  }
